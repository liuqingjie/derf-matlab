function  descriptors=assemblingdescriptors_multi_all(I,sviv,ogrid,ostable,o,ii,jj)   
    ...
    for i=1:sviv.SQ  %% 
         ...
         for j=1:length(current_features) % 
              ...
              if i==1
                   ...
                   for m=0:sviv.numofver*(sviv.SQ-1)-1
                   ...
                   sample_jj4(oob)=1;
                       ...
                       for m=1:sviv.numofver*(sviv.SQ-1)
                            descriptors(:,    (current_features(j) - 1 )*(sviv.SQ-1)*sviv.numofver + m, channel)=sviv.H(sample_ii1+(sample_jj1-1)*rows+(channel-1)*rows*cols+( m-1)*rows*cols*chnum).*(1-a).*(1-b)   +   sviv.H(sample_ii2+(sample_jj2-1)*rows+(channel-1)*rows*cols+(m-1)*rows*cols*chnum).*b.*(1-a)  +  sviv.H(sample_ii3+(sample_jj3-1)*rows+(channel-1)*rows*cols+(m-1)*rows*cols*chnum).*(1-b).*(a)   +   sviv.H(sample_ii4+(sample_jj4-1)*rows+(channel-1)*rows*cols+(m-1)*rows*cols*chnum).*(b).*(a);
                            descriptors(oob,  (current_features(j) - 1)*(sviv.SQ-1)*sviv.numofver  + m, channel)=0;
                       end
